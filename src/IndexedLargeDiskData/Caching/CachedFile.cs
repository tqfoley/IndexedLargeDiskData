...
public sealed class CachedFile : IDisposable
{
    ...
    public void SetLength(long length) => Interlocked.Exchange(ref _length, length);
    ...
    public void Invalidate(long blockIndex) => _cache.Invalidate(Id, blockIndex);

    /// <summary>Acquires a pinned lease on one block, reading it from disk if it is not cached.</summary>
    /// <param name="blockIndex">The zero-based block index within this file.</param>
    public BlockLease Acquire(long blockIndex) => _cache.Acquire(this, blockIndex);
